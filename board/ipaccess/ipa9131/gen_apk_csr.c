...
#define SIZE_OF_CA_KEY_IN_BYTES 256
u8 oem_real_ca_pub_mod[SIZE_OF_CA_KEY_IN_BYTES] = { 0xbb,0x3b,0x97,0x70,0x3e,0x1f,0x51,0x80,0x7e,0xff,0x45,0x25,0xe4,0xe7,0x45,0xe7,0xac,0x42,0x45,0x36,0x26,0x78,0x1e,0x6b,0xc6,0xca,0x02,0x64,0x5d,
...
int oem_real_ca_pub_exp = 50457;

u8 oem_tst_ca_pub_mod[SIZE_OF_CA_KEY_IN_BYTES] = {  };
int oem_tst_ca_pub_exp = 39981;
...
static int oem_rsa_encrypt(uint8_t *in_buf,uint32_t in_len,uint8_t *out_buf,uint32_t *out_len )
{
	int ret = 0;
	rsa_context rsa;
	u8 *pub_mod = oem_real_ca_pub_mod;
	int *pub_exp = &oem_real_ca_pub_exp;

	if ( ! characterisation_is_production_mode() )
	{
            pub_mod = oem_tst_ca_pub_mod;
            pub_exp = &oem_tst_ca_pub_exp;
	}

	rsa_init( &rsa, RSA_PKCS_V15, 0 );

	if ( (ret = mpi_read_binary( &rsa.N, pub_mod, SIZE_OF_CA_KEY_IN_BYTES )) != 0 )
	{
	...
	if ( (ret = mpi_lset(&rsa.E,*pub_exp)) != 0 )
	{
...
int get_signature( const uint8_t *pub_key_buf, uint32_t pub_key_len ,const uint8_t *hash,uint32_t hash_len, crypt_buf_t *sig_buf )
{
    int ret = 0;
    pk_context key;
    unsigned char *out_buf;
    unsigned int olen = KEY_SIZE_BYTES;

    pk_init( &key );
    if ( !(out_buf = (unsigned char *)calloc(olen, sizeof(unsigned char))) )
    {
        ret = -errno;
        goto end;
    }
    ...
    if( 0 != (ret = pk_parse_public_key( &key ,(const unsigned char *) pub_key_buf, (size_t) pub_key_len )) )
    {
        fprintf( stderr,"failed parse_public_key returned 0x%08X\n", ret );
        goto end;
    }
    ...
    if( 0 != (ret = pk_sign( &key, POLARSSL_MD_SHA256, hash, 0, out_buf, &olen,
                    gen_rand_num, NULL )) )
    {
        fprintf( stderr," failed pk_sign returned -0x%04x\n", -ret );
        goto end;
    }
