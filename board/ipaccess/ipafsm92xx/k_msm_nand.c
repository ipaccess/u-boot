...
#include "malloc.h"
#include "k_msm_nand.h"
#include "k_msm_nand_port.h"
#include "common.h"
#include "asm/bitops.h"
#include "asm-generic/errno.h"
#include "k_mtd.h"
#include "k_mtd-abi.h"
#include "k_dma-fsm9xxx.h"
#include "k_dma.h"
#include "malloc.h"
...
static void *msm_nand_get_dma_buffer(struct msm_nand_chip *chip, size_t size)
{
    ...
    //printf("%s dma_buffer_busy:0x%x\n",__func__,chip->dma_buffer_busy.counter);
					...
					void *buffer, size_t size)
{
    ...
    // printf("%s buffer:%p dma_buffer_busy:0x%x\n",__func__,buffer,chip->dma_buffer_busy.counter);
...
uint32_t flash_read_id(struct msm_nand_chip *chip)
{
	...
	//printf("%s status: %x\n",__func__, dma_buffer->data[3]);
	debug("%s nandid: %x maker %02x device %02x\n",__func__,
...
uint32_t flash_onfi_probe(struct msm_nand_chip *chip)
{
    ...
    //int i;              // Temp for debug
    //uint32_t * ptmp;    // Temp for debug

    //printf("%s msm_nand_data.nr_parts:%d \n",__func__,msm_nand_data.nr_parts);
    //We don't care about partitions, flash starts at 0.
#if 0
...
#else
    //TODO Hacky, found by looking at linux boot output
    page_address = (0x0c100000 << 6);
#endif
    ...
    //printf("%s after get dma buffer 1 dma_addr_identifier:0x%x onfi_identifier_buf:%p\n",
    //        __func__,dma_addr_identifier, onfi_identifier_buf);
    ...
    //printf("%s after get dma buffer 2 dma_addr_param_info:0x%x onfi_param_info_buf:%p\n",
    //        __func__,dma_addr_param_info,onfi_param_info_buf);

	wait_event(chip->wait_queue, (dma_buffer = msm_nand_get_dma_buffer
				(chip, sizeof(*dma_buffer))));
    //printf("%s after get dma buffer 3 dma_buffer:%p\n",__func__,dma_buffer);

	dma_buffer->data.sflash_bcfg_orig = flash_rd_reg
				(chip, MSM_NAND_SFLASHC_BURST_CFG);
    //printf("%s sflash_bcfg_orig:0x%x\n",__func__,dma_buffer->data.sflash_bcfg_orig);

	dma_buffer->data.devcmd1_orig = flash_rd_reg(chip, MSM_NAND_DEV_CMD1);
    //printf("%s devcmd1_orig:0x%x\n",__func__,dma_buffer->data.devcmd1_orig);

	dma_buffer->data.devcmdvld_orig = flash_rd_reg(chip,
						 MSM_NAND_DEV_CMD_VLD);
    //printf("%s devcmdvld_orig:0x%x\n",__func__,dma_buffer->data.devcmdvld_orig);
	...
	while (cmd_set_count-- > 0) {
        ...
        //printf("%s devcmd1_mod:0x%x\n",__func__,dma_buffer->data.devcmd1_mod);
        ...
        //printf("%s dma_buffer->data array\n",__func__);
        //ptmp = (uint32_t*)&dma_buffer->data;
        //for(i=0; i<13; i++)
		...
		cmd->len = 12;
        //printf("cmd 2 cmd:0x%x\n",cmd->cmd);
        //printf("cmd 2 src:0x%x\n",cmd->src);
        //printf("cmd 2 dst:0x%x\n",cmd->dst);
        //printf("cmd 2 len:0x%x\n",cmd->len);
		cmd++;
        ...
        //printf("%s chip->dma_buffer %p\n",__func__,chip->dma_buffer);
        //printf("%s dma_buf:%p cmdptr:0x%x\n",__func__,dma_buffer,dma_buffer->cmdptr);
		mb();
        ...
        //printf("%s cmd_set_count:%d\n",__func__,cmd_set_count);
		if (cmd_set_count) {
			onfi_param_page_ptr = (struct onfi_param_page *)
				(&(onfi_identifier_buf[0]));
            //printf("%s parameter_page_signature:0x%x\n",__func__,onfi_param_page_ptr->parameter_page_signature);
            ...
            //printf("%s crc_chk_count:%d \n",__func__,crc_chk_count);
			if (crc_chk_count >= ONFI_PARAM_INFO_LENGTH
				...
				supported_flash.flash_id =
					flash_read_id(chip);
                //printf("%s flash_id:0x%x\n",__func__,supported_flash.flash_id);
				...
				debug("%s ONFI probe : Found an ONFI "
					"compliant device %s\n",__func__,
			     ...
			     struct mtd_oob_ops *ops)
{
    ...
    //printf("%s page:0x%x %d\n",__func__,page,page);
	if (ops->mode != MTD_OPS_RAW) {
		if (ops->datbuf != NULL && (ops->len % mtd->writesize) != 0) {
			/* when ops->datbuf is NULL, ops->len can be ooblen */
			pr_err("%s: unsupported ops->len: %d mtd->writesize: %d\n",
			       __func__, ops->len, mtd->writesize);
			return -EINVAL;
		}
	} else {
		if (ops->datbuf != NULL &&
			...
			pr_err("%s: unsupported ops->len,"
				" %d for MTD_OPS_RAW\n", __func__, ops->len);
            printf("%s writesize:%d oobsize:%d\n",__func__,mtd->writesize,mtd->oobsize);
			return -EINVAL;
		}
	}
	...
	if (ops->datbuf) {
        //printf("%s datbuf:%p len:%d\n",__func__,ops->datbuf,ops->len);
	...
	if (ops->oobbuf) {
        //printf("%s oobbuf:%p len:%d\n",__func__,ops->oobbuf,ops->ooblen);
	...
	while (page_count-- > 0) {
        ...
        //printf("%s addr0:0x%x addr1:0x%x page:%d oob_col:%d \n",__func__,
        //        dma_buffer->data.addr0,
        //        dma_buffer->data.addr1,
        //        page,
        //        oob_col);
        ...
        //printf("%s start_sector:%d cwperpage:%d \n",__func__,start_sector,cwperpage);
		for (n = start_sector; n < cwperpage; n++) {
			 ...
			 */
			cmd->cmd = DST_CRCI_NAND_CMD;
            //printf("%s  DST_CRCI_NAND_CMD cmd 0x%x\n",__func__,cmd->cmd);
			cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.cmd);
			cmd->dst = MSM_NAND_FLASH_CMD;
			if (n == start_sector)
				cmd->len = 16;
			 ...
			 */
			cmd->cmd = SRC_CRCI_NAND_DATA;
            //printf("%s  SRC_CRCI_NAND_DATA cmd 0x%x\n",__func__,cmd->cmd);
			cmd->src = MSM_NAND_FLASH_STATUS;
            //printf("%s  MSM_NAND_FLASH_STATUS src 0x%x\n",__func__,cmd->src);
			cmd->dst = msm_virt_to_dma(chip,
			...
			if (ops->datbuf) {
                ...
                //printf("%s sectordatasize:%d cw_size:%d\n",__func__,sectordatasize,chip->cw_size);
				cmd->cmd = 0;
				cmd->src = MSM_NAND_FLASH_BUFFER;
				cmd->dst = data_dma_addr_curr;
                //printf("%s read data block dst:0x%x\n",__func__,cmd->dst);
				data_dma_addr_curr += sectordatasize;
				cmd->len = sectordatasize;
                //printf("%s with datbuf cmd->len:%d\n",__func__,cmd->len);
				cmd++;
			}
			...
			if (ops->oobbuf && (n == (cwperpage - 1)
                ...
                //printf("%s read oob dst:%p sectoroobsize:%d ecc_parity_bytes:%d\n",
                //        __func__,cmd->dst,sectoroobsize, chip->ecc_parity_bytes);
				if (sectoroobsize < oob_len)
					cmd->len = sectoroobsize;
				else
					cmd->len = oob_len;
                //printf("%s with oobbuf cmd->len:%d\n",__func__,cmd->len);
        ...
        //printf("%s start_sector:%d cwperpage:%d rawerr:%d\n",__func__,start_sector,cwperpage,rawerr);
		if (rawerr) {
		...
		if (!rawerr) { /* check for corretable errors */
			for (n = start_sector; n < cwperpage; n++) {
				...
				(dma_buffer->data.result[n].buffer_status
				 & chip->num_err_mask);
                //printf("%s n:%d ecc_errors:0x%x\n",__func__,n,ecc_errors);
				if (ecc_errors) {
		...
		pages_read++;
		page++;
        //printf("%s next pages_read:%d page:%d\n",__func__,pages_read,page);
	}
	msm_nand_release_dma_buffer(chip, dma_buffer, sizeof(*dma_buffer));
	      ...
	      size_t *retlen, u_char *buf)
{
    ...
    //printf("%s from:0x%llx len:0x%ld \n",__func__,from,len);
	...
	*retlen = 0;
	...
	if ((from & (mtd->writesize - 1)) == 0 && len == mtd->writesize) {
        ...
        //printf("%s page on page boundary\n",__func__);
		ops.len = len;
		ops.datbuf = buf;
...
#define WITH_ZEROED_PAGE
static uint8_t* zeroed_page(void)
{
    int i;
    uint8_t* ret;

    ret = memalign(ARCH_DMA_MINALIGN, PAGE_SIZE);
    if(!ret)
    {
        printf("%s ERROR failed allocation\n",__func__);
        return 0;
    }
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (dev_found) {
		...
		if (msm_nand_has_bch_ecc_engine(hw_id)
			...
			debug("Found supported NAND device for %dbit ECC\n",
					supported_flash.ecc_correctability);
			enable_bch_ecc = 1;
		} else {
			debug("Found a supported NAND device\n");
		}
		debug("NAND Controller ID : 0x%x\n", hw_id);
		debug("NAND Device ID  : 0x%x\n", supported_flash.flash_id);
		debug("Buswidth : %d Bits\n", (wide_bus) ? 16 : 8);
		debug("Density  : %lld MByte\n", (mtd->size>>20));
		debug("Pagesize : %d Bytes\n", mtd->writesize);
		debug("Erasesize: %d Bytes\n", mtd->erasesize);
		debug("Oobsize  : %d Bytes\n", mtd->oobsize);
	} else {
...
#if 0
#warning "Testing disable HW ECC"
    chip->CFG1 = (1 << 0)  /* disable HW ECC */
#else
...
#warning "Testing 4bit ecc"
	...
	debug("CFG0 Init  : 0x%08x\n", chip->CFG0);
	debug("CFG1 Init  : 0x%08x\n", chip->CFG1);
	debug("ECCBUFCFG  : 0x%08x\n", chip->ecc_buf_cfg);
...
int msm_nand_probe(struct mtd_info *mtd)
{
    ...
    plat_data = &msm_nand_data;

    msm_nand_phys       = 0x81600000;
    msm_nandc01_phys    = 0;
    msm_nandc10_phys    = 0;
    msm_nandc11_phys    = 0;
    ebi2_register_base  = 0;
    interleave_enable   = 0;
	...
	info->msm_nand.dma_channel = 7;
	debug("%s: dmac 0x%x\n", __func__, info->msm_nand.dma_channel);

	info->msm_nand.dma_buffer = memalign(32, MSM_NAND_DMA_BUFFER_SIZE);
    info->msm_nand.dma_addr = (u32)info->msm_nand.dma_buffer;
	...
	debug("%s: allocated dma buffer at %p, %x -> %x \n",
		__func__, info->msm_nand.dma_buffer, info->msm_nand.dma_addr, info->msm_nand.dma_addr + MSM_NAND_DMA_BUFFER_SIZE);
    ...
    mtd->name = "MTD_DEVICE_NAME";
    mtd->priv = &info->msm_nand;
    mtd->owner = THIS_MODULE;

    msm_dmov_probe();

    if (msm_nand_scan(mtd, 1))
    {
        if (msm_onenand_scan(mtd, 1))
        {
