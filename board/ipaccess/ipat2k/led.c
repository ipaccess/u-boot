...
#define LED_NWK_GREEN_GPIO          26 
#define LED_NWK_RED_INDEX           1
#define LED_NWK_RED_GPIO            27

/* LED_SVC */
#define LED_SVC_GREEN_INDEX         2
#define LED_SVC_GREEN_GPIO          9
#define LED_SVC_RED_INDEX           3
#define LED_SVC_RED_GPIO            4

/*LED POWER*/
#define LED_PWR_GREEN_INDEX         4
#define LED_PWR_GREEN_GPIO          19
...
static int simple_set_gpio(unsigned int gpio_num, unsigned int value)
{
    ...
    REG32(GPIO_OE_REG)      |= (1 << gpio_num);
    if (value)
    {
        REG32(GPIO_OUTPUT_REG)  |= (1 << gpio_num);
    }
    else
    {
        REG32(GPIO_OUTPUT_REG)  &= ~(1<< gpio_num);
...
int setLED(led_index index, led_colour colour)
{
    ...
    if ( (index < NUM_LEDS) && (colour < NUM_COLOURS) )
    {
        ...
        for (i = 0; i < GPIO_LINES_PER_LED; ++i)
        {
            ...
            /*Only one GPIO for power led*/
            if((LED_PWR_GREEN_INDEX == gpio_index) || (LED_PWR_RED_INDEX == gpio_index))
            {
            	set_gpio(gpio_index, (setting & 0x1) ? GPIO_LO : GPIO_HI);
                break;
            }
            else
            {
