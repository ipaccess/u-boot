...
int gem_start_tx(struct gemac_dev *gemac)
{
	...
	REG32(gemac->registers + GEM_NET_CONTROL) |= (GEM_NETCTRL_START_TX | GEM_NETCTRL_TX_ENB);
#endif
	return 0;
}
...
#ifdef CONFIG_T2K
	if (gem_loop) {
		//TODO fix driver loopback function
		REG32(gemac->registers + GEM_NET_CONFIG) &= ~GEM_PCS_SEL; // should be GEM_NETCFG_TBI_ENB (1 << 11)
		REG32(gemac->registers + GEM_NET_CONTROL) |= GEM_NETCTRL_RX_LB;
	} else {
		REG32(gemac->registers + GEM_NET_CONTROL) &= ~GEM_NETCTRL_RX_LB;
	}
#else
...
{
	// Set GEM internal duplex mode, if duplex mode controlled
	// by GEM instead of automatically set and reported by PHY
	// via optional in-band duplex status messages
	*(volatile u32 *)(gemac->baseaddr + GEM_CFG) |= GEM_CONF_DUPLEX_GEM_FULL;
#if defined(CONFIG_T2K)
    *(volatile u32 *)(gemac->registers + GEM_NET_CONFIG ) |= GEM_FULL_DUPLEX;
#endif
}
...
void gem_set_speed(struct gemac_dev *gemac, MAC_SPEED gem_speed)
{
...
#else
		*(volatile u32*)(gemac->registers + GEM_NET_CONFIG ) &= ~(GEM_PCS_SEL | GEM_GIG_MODE | GEM_SPEED_100);
#endif
		break;
	...
	case SPEED_100M:
		...
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG ) &= ~(GEM_PCS_SEL);
#else
		*(volatile u32*)(gemac->registers + GEM_NET_CONFIG ) &= ~(GEM_PCS_SEL | GEM_GIG_MODE);
		*(volatile u32*)(gemac->registers + GEM_NET_CONFIG ) |= GEM_SPEED_100;
#endif
		break;
	default:
	case SPEED_1000M:
		...
		*(volatile u32 *)(gemac->registers + GEM_NET_CONFIG) &= (~GEM_PCS_SEL);
#else
		*(volatile u32*)(gemac->registers + GEM_NET_CONFIG ) &= ~(GEM_PCS_SEL | GEM_SPEED_100);
		*(volatile u32*)(gemac->registers + GEM_NET_CONFIG ) |= GEM_GIG_MODE;
#endif
		break;
...
MAC_SPEED gem_get_speed(struct gemac_dev *gemac)
{
	...
	if (*(volatile u32 *)(gemac->baseaddr + GEM_CFG) & GEM_CONF_SPEED_SEL_GEM) {
		// GEM speed is set with GEM in control instead of PHY, get speed from GEM configuration register
		...
		// GEM speed is controlled by the PHY, not the GEM, read the speed from the PHY status bits of the GEM configuation register
...
/******************************************************************************/
#if defined(CONFIG_COMCERTO_100) || defined(CONFIG_T2K)
/******************************************************************************/
/*
...
int gem_reset(struct gemac_dev *gemac)
{
...
#ifdef CONFIG_T2K
	gem_set_ahb_width(gemac, AMBA_AHB_64);
#endif
...
#else
	*(volatile u32 *)(gemac->registers + GEM_RX_QUEUE0) = 0x00000000;
#endif
	...
	*(volatile u32 *)(gemac->registers + GEM_ID_CHECK4) = 0x00000000;
	...
	if ((*(volatile u32 *)(gemac->registers + GEM_ID_CHECK1) != 0x00000000) ||
	    ...
	    (*(volatile u32 *)(gemac->registers + GEM_IRQ_STATUS) != 0x00000000) ||
	// TODO: fix in model (it returns 0x7fff ffff)
/*
#ifndef CONFIG_T2K
    	(*(volatile u32 *)(gemac->registers + GEM_IRQ_MASK) != 0x3FFFFFFF) ||
#endif
