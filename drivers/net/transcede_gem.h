...
#define MDIO_TIMEOUT            2	/* instruction cycles? */
#else
...
//
// GEMAC configuration bits, used for configuration options
// and also setup for the same values as used in the Transcede
// GEMCORE configuration register (offset 0xF000)
//
#define GEMAC_PIN_MODE		(0 << 0)		// GEMAC configuration based on external pins
#define GEMAC_GEM_MODE		(1 << 0)		// GEMAC configuration based on register settings
#define GEMAC_GEM_MODE_RGMII	(0 << 3)		// GEMAC configuration RGMII (if bit 0 is 1)
#define GEMAC_GEM_MODE_SGMII	(1 << 3)		// GEMAC configuration SGMII (if bit 0 is 1)
#define GEMAC_SW_CONF		(1 << 8) | (1 << 11)    // GEMAC configured by SW
#define GEMAC_SW_CONF_SPEED	(1 << 11)		// GEMAC Speed configured by SW
#define GEMAC_SW_CONF_DUPLEX    (1 << 8)		// GEMAC Duplex (half or full) configured by SW
#define GEMAC_PHY_CONF		(0 << 8) | (0 << 11)    // GEMAC configured by phy lines (RGMII mode only)
#define GEMAC_SW_HALF_DUPLEX	(0 << 9)
...
#define CONFIG_COMCERTO_USE_SGMII	(1 << 4)
...
{
       volatile u32     Cfg;        // GEMCORE Configuration Register
       volatile u32     MBIST;      // GEMCORE MBIST Register
       volatile u32     Sense;      // GEMCORE Sense Amp Delay Adjustment Register
       volatile u32     Delay;      // GEMCORE Delay Element Control Register
...
typedef struct tGEMSTATREGS
{
    ...
    // TX statistics  (Tx)
    // -----------------------------------------------
    volatile u64     TxBytes;            // Octets transmitted [47:0] (in frames without error)
    volatile u32     TxFrames;           // Frames transmitted without error, excluding pause frames
    volatile u32     TxBcstFrames;       // Broadcast frames transmitted without error, excluding pause frames
    volatile u32     TxMcstFrames;       // Multicast frames transmitted without error,excluding pause frames
    volatile u32     TxPauseFrames;      // Transmitted pause frames (unless sent through external FIFO interface)
    volatile u32     Tx64Frames;         // 64 byte frames transmitted without error, excluding pause frames
    volatile u32     Tx65_127Frames;     // 65 to 127 byte frames transmitted without error
    volatile u32     Tx128_255Frames;    // 128 to 255 byte frames transmitted without error
    volatile u32     Tx256_511Frames;    // 256 to 511 byte frames transmitted without error
    volatile u32     Tx512_1023Frames;   // 512 to 1023 byte frames transmitted without error
    volatile u32     Tx1024_1518Frames;  // 1024 to 1518 byte frames transmitted without error
    volatile u32     TxGt1518Frames;     // Greater than 1518 byte frames transmitted without error
    volatile u32     TxUnderrunErr;      // Transmit under run errors
    volatile u32     TxSingleCollFrames; // Single collision frames
    volatile u32     TxMultCollFrames;   // Multiple collision frames
    volatile u32     TxExcCollFrames;    // Excessive collisions
    volatile u32     TxLateCollFrames;   // Late collisions
    volatile u32     TxDeferredFrames;   // Deferred transmission frames
    volatile u32     TxCarrierSenseErr;  // Carrier sense errors
    // -----------------------------------------------
    // RX statistics  (Rx)
    // -----------------------------------------------
    volatile u64     RxBytes;            // Octets received [47:0] (in frames without error)
    volatile u32     RxFrames;           // Frames received without error, excluding pause frames
    volatile u32     RxBcstFrames;       // Broadcast frames received without error, excluding pause frames
    volatile u32     RxMcstFrames;       // Multicast frames received without error,excluding pause frames
    volatile u32     RxPauseFrames;      // received pause frames (unless sent through external FIFO interface)
    volatile u32     Rx64Frames;         // 64 byte frames transmitted without error, excluding pause frames
    volatile u32     Rx65_127Frames;     // 65 to 127 byte frames received without error
    volatile u32     Rx128_255Frames;    // 128 to 255 byte frames received without error
    volatile u32     Rx256_511Frames;    // 256 to 511 byte frames received without error
    volatile u32     Rx512_1023Frames;   // 512 to 1023 byte frames received without error
    volatile u32     Rx1024_1518Frames;  // 1024 to 1518 byte frames received without error
    volatile u32     RxGt1518Frames;     // Greater than 1518 byte frames received without error
    volatile u32     RxUndersizeFrames;  // Undersize frames received
    volatile u32     RxOversizeFrames;   // Oversize frames received
    volatile u32     RxJabbersFrames;    // Jabbers received
    volatile u32     RxFrmChkSeqErr;     // Frame check sequence errors
    volatile u32     RxLenErr;           // Length field frame errors
    volatile u32     RxRecvSymErr;       // Receive symbol errors
    volatile u32     RxAlignErr;         // Alignment errors
    volatile u32     RxRecvResErr;       // Receive resource errors
    volatile u32     RxRecvOvrErr;       // Receive overrun errors
    volatile u32     RxIPCksumErr;       // IP header checksum errors
    volatile u32     RxTCPCksumErr;      // TCP checksum errors
    volatile u32     RxUDPCksumErr;      // UDP checksum errors
...
typedef struct tGEMIPREGS
{
    ...
    volatile u32 NetCtrl;            // Network control
    volatile u32 NetCfg;             // Network configuration
    volatile u32 NetStat;            // Network status
    volatile u32 UserIO;             // User input/output
    volatile u32 DMACfg;             // DMA configuration
    volatile u32 TxStatus;           // Transmit status
    volatile u32 RxQueue;            // Receive buffer queue base address

    volatile u32 RxGlobalOffs;       // RX Global Offset

    volatile u32 RxStatus;           // Receive status
    volatile u32 IntStat;            // Interrupt status
    volatile u32 IntEnable;          // Interrupt enable
    volatile u32 IntDisable;         // Interrupt disable
    volatile u32 IntMask;            // Interrupt mask

    // 0x34 offset
    volatile u32 PHY;                // PHY maintenance

    volatile u32 RxPauseQuantum;     // Received pause quantum
    volatile u32 TxPauseQuantum;     // Transmit pause quantum

    volatile u32 Res1[16];

    // 0x80 offset

    volatile u32 HashRegBottom;      // Hash register bottom [31:0]
    volatile u32 HashRegTop;         // Hash register top [63:32]
    volatile u32 SpecAddr1Bottom;    // Specific address 1 bottom [31:0]
    volatile u32 SpecAddr1Top;       // Specific address 1 top [47:32]
    volatile u32 SpecAddr2Bottom;    // Specific address 2 bottom [31:0]
    volatile u32 SpecAddr2Top;       // Specific address 2 top [47:32]
    volatile u32 SpecAddr3Bottom;    // Specific address 3 bottom [31:0]
    volatile u32 SpecAddr3Top;       // Specific address 3 top [47:32]
    volatile u32 SpecAddr4Bottom;    // Specific address 4 bottom [31:0]
    volatile u32 SpecAddr4Top;       // Specific address 4 top [47:32]

    // 0xA8 offset
    volatile u32 TypeID [4];

    // 0xB8 offset
    volatile u32 WakeOnLAN;          // Wake on LAN

    // 0xBC offset
    volatile u32 IPGStretch;         // IPG stretch
    volatile u32 StackedVLAN;        // Stacked VLAN

    volatile u32 Res2 [14];

    // 0xFC offset
    volatile u32 ModuleID;           // Module ID
    ...
    // The offset is 0x1D0
    volatile u32 TimerSec;           // 1588 timer seconds
    volatile u32 TimerNanosec;       // 1588 timer nanoseconds
    volatile u32 TimerAdjust;        // 1588 timer adjust
    volatile u32 TimerIncr;          // 1588 timer increment

    // The offset is 0x1E0
    volatile u32 PTPTxSec;           // PTP event frame transmitted seconds
    volatile u32 PTPTxNanoSec;       // PTP event frame transmitted nanoseconds
    volatile u32 PTPRxSec;           // PTP event frame received seconds
    volatile u32 PTPRxNanoSec;       // PTP event frame received nanoseconds
    volatile u32 PTPPeerTxSec;       // PTP peer event frame transmitted seconds
    volatile u32 PTPPeerTxNanoSec;   // PTP peer event frame transmitted nanoseconds
    volatile u32 PTPPeerRxSec;       // PTP peer event frame received seconds
    volatile u32 PTPPeerRxNanoSec;   // PTP peer event frame received nanoseconds

    // The offset is 0x200
    volatile u32 PCSCtrl;            // PCS control
    volatile u32 PCSStat;            // PCS status
    volatile u32 PCSHighPhyID;       // PCS upper PHY identifier
    volatile u32 PCSLowPhyID;        // PCS lower PHY identifier
    volatile u32 PCSAdv;             // PCS auto-negotiation advertisement
    volatile u32 PCSLink;            // PCS auto-negotiation link partner ability
    volatile u32 PCSExp;             // PCS auto-negotiation expansion
    volatile u32 PCSNextPage;        // PCS auto-negotiation next page
    volatile u32 PCSLinkNextPage;    // PCS auto-negotiation link partner next page

    volatile u32 Res4[6];

    // Offset 0x23C
    volatile u32 PCSExtStat;         // PCS extended status

    // Offset 0x240
    volatile u32 Res5[48];

    // Offset 0x300
    volatile u32 TxQueue[8];         // Transmit buffer queue base address

    // Additional TypeID,
    // offset is 0x320
    volatile u32 ExtTypeID[4];      // Type ID match 5 ... Type ID match 8

    // Offset 0x330
    volatile u32 TXPBUF;             // TX PBUF watermark

    // Offset 0x334
    volatile u32 TSUStatus;
    volatile u32 TSUEnable;
    volatile u32 TSUDisable;
    volatile u32 TSUMask;

    u32         dummy[303];
} GEMIPREGS, *PGEMIPREGS;

typedef struct tGEMSCHREGS
{
    volatile u32 sch_status;
    volatile u32 sch_ctrl;
    volatile u32 sch_pkt_queued;
} GEMSCHREGS, *PGEMSCHREGS;
...
typedef struct tGEMTXBDESC
{
    u8*     BPtr;           // The address + some options bits used by the DMA
    u32     Descr;          // The status, flags and parameters of DMA buffer descriptor

} GEMTXBDESC, *PGEMTXBDESC;
...
typedef struct tGEMRXBDESC
{
    u8*     BPtr;           // The address DMA
    u32     Descr;          // The status, flags and parameters of DMA buffer descriptor
#ifndef CONFIG_T2K
    u32     ExDescr;        // The extended status, flags and parameters of DMA buffer descriptor
    u32     Pad;
#endif
} GEMRXBDESC, *PGEMRXBDESC;

/******************************************************************************
*       Definition of the GEM context type                                    *
*******************************************************************************/

typedef void (*GemDevIrqCB)(void* pCBCtx, void* pGemDrvCtx, u32 nIrqStatus);


typedef struct tagETHCTX
{
    PGEMRXBDESC     pRxDesc;
    PGEMTXBDESC     pTxDesc;
    PGEMTXBDESC     pTailTxDesc;
    PGEMIPREGS      GemIPRegs;
    PGEMSCHREGS     GemSchRegs;
    PGEMCOREREGS    GemCoreRegs;
    PGEMRXBDESC     pRxCurDesc;

    GemDevIrqCB     IrqProc;
    void*           ProcCtx;
    u32             IrqMask;
    u8              MacAddr[6];
    GEMSTATREGS     Statistics;
} ETHCTX, *PETHCTX;
