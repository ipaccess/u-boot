...
#else
    sha256_starts_mbed( &ctx->accumulator, 0 );
#endif
#if defined(POLARSSL_HAVEGE_C)
...
#else
    sha256_update_mbed( &ctx->accumulator, header, 2 );
    sha256_update_mbed( &ctx->accumulator, p, use_len );
#endif
...
int entropy_func( void *data, unsigned char *output, size_t len )
{
    ...
    sha256_finish_mbed( &ctx->accumulator, buf );
    ...
    sha256_starts_mbed( &ctx->accumulator, 0 );
    sha256_update_mbed( &ctx->accumulator, buf, ENTROPY_BLOCK_SIZE );
