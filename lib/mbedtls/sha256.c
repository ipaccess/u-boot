 ...
 */
void sha256_starts_mbed( sha256_context *ctx, int is224 )
{
    ctx->total[0] = 0;
    ctx->total[1] = 0;
 ...
 */
void sha256_update_mbed( sha256_context *ctx, const unsigned char *input,
                    size_t ilen )
{
    size_t fill;
    uint32_t left;
 ...
 */
void sha256_finish_mbed( sha256_context *ctx, unsigned char output[32] )
{
    uint32_t last, padn;
    uint32_t high, low;
    ...
    sha256_update_mbed( ctx, sha256_padding, padn );
    sha256_update_mbed( ctx, msglen, 8 );
             ...
             unsigned char output[32], int is224 )
{
    ...
    sha256_init( &ctx );
    sha256_starts_mbed( &ctx, is224 );
    sha256_update_mbed( &ctx, input, ilen );
    sha256_finish_mbed( &ctx, output );
    sha256_free( &ctx );
}
                         ...
                         size_t keylen, int is224 )
{
    ...
    sha256_starts_mbed( ctx, is224 );
    sha256_update_mbed( ctx, ctx->ipad, 64 );
...
void sha256_hmac_finish( sha256_context *ctx, unsigned char output[32] )
{
    ...
    sha256_finish_mbed( ctx, tmpbuf );
    sha256_starts_mbed( ctx, is224 );
    sha256_update_mbed( ctx, ctx->opad, 64 );
    sha256_update_mbed( ctx, tmpbuf, hlen );
    sha256_finish_mbed( ctx, output );
...
{
    sha256_starts_mbed( ctx, ctx->is224 );
    sha256_update_mbed( ctx, ctx->ipad, 64 );
}
...
int sha256_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( j == 2 )
        {
            ...
            for( j = 0; j < 1000; j++ )
                sha256_update_mbed( &ctx, buf, buflen );
        }
        else
            sha256_update_mbed( &ctx, sha256_test_buf[j],
                                 sha256_test_buflen[j] );

        sha256_finish_mbed( &ctx, sha256sum );
