...
#if defined(POLARSSL_DES_C)
static int pem_des3_encrypt( unsigned char *iv,
		unsigned char *buf, size_t *buflen,
		const unsigned char *pwd, size_t pwdlen )
{
	unsigned int i = 0;
	...
	/*Add pkcs#7 style padding here*/        
	pad_len = 8 - pad_len;

        for (i=0;i < pad_len; i++)
        {
            buf[*buflen + i] = pad_len;
        }
	...
	*buflen +=pad_len; 
	...
	des3_set3key_enc( &des3_ctx, des3_key );
	ret = des3_crypt_cbc( &des3_ctx, DES_ENCRYPT, *buflen,
			des3_iv, buf, buf );
...
#if defined(POLARSSL_PEM_WRITE_C) && defined(POLARSSL_MD5_C) && defined(POLARSSL_CIPHER_MODE_CBC) && defined(POLARSSL_DES_C)
int pem_write_buffer_enc_des3( const char *header, const char *footer,
		const unsigned char *der_data, size_t der_len,
		unsigned char *buf, size_t buf_len, size_t *olen,const unsigned char * pwd, unsigned char *iv,size_t iv_len )
{
	int ret = 0, i = 0;
	unsigned char *encode_buf, *c, *p = buf;
	size_t len = 0, use_len = 0, add_len = 0,out_len = 0;
	unsigned char *des_buf=NULL;
	char *enc_header="Proc-Type: 4,ENCRYPTED";
	char dek_info[100];
	char iv_str[17];

        memset(iv_str,0,17);
        memset(dek_info,0,100);


	if( ( des_buf = polarssl_malloc( der_len + 16 ) ) == NULL )
		return( POLARSSL_ERR_PEM_MALLOC_FAILED );
	
	
	memcpy( des_buf, der_data, der_len);


	if ( (ret = pem_des3_encrypt(iv,des_buf,&der_len,pwd,strlen(pwd))) != 0 )
        {
            return (ret);
        }

	base64_encode( NULL, &use_len, des_buf, der_len );

        for(i=0;i < 8;i++)
        {
            snprintf((iv_str + 2*i),2*sizeof(char),"%02X",iv[i]);

        }

	snprintf(dek_info,sizeof(dek_info),"DEK-Info: DES-EDE3-CBC,%s",iv_str);

	add_len = strlen( header ) + strlen( footer ) + ( use_len / 64 ) + 1 + strlen(enc_header) + strlen(dek_info);

	if( use_len + add_len > buf_len )
	{
		*olen = use_len + add_len;
                polarssl_free( des_buf );
		return( POLARSSL_ERR_BASE64_BUFFER_TOO_SMALL );
	}
	...
	if( ( ret = base64_encode( encode_buf, &use_len, des_buf,
					der_len ) ) != 0 )
	{
		polarssl_free( encode_buf );
		polarssl_free( des_buf );
	...
	memcpy( p, enc_header, strlen( enc_header ) );
	p += strlen( enc_header );
        *p++='\n';

	memcpy( p, dek_info, strlen( dek_info ) );
	p += strlen( dek_info );
	*p++='\n';
	*p++='\n';	
...
#endif /* POLARSSL_PEM_WRITE_C && POLARSSL_MD5_C && POLARSSL_CIPHER_MODE_CBC && POLARSSL_DES_C*/
