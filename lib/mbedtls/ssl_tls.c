                     ...
                     unsigned char *dstbuf, size_t dlen )
{
    ...
    for( i = 0; i < dlen / 16; i++ )
    {
        ...
        sha1_starts_mbed( &sha1 );
        sha1_update_mbed( &sha1, padding, 1 + i );
        sha1_update_mbed( &sha1, secret, slen );
        sha1_update_mbed( &sha1, random, rlen );
        sha1_finish_mbed( &sha1, sha1sum );
                     ...
                     unsigned char *dstbuf, size_t dlen )
{
     ...
     */
    sha1_hmac_mbed( S2, hs, tmp + 20, nb, tmp );

    for( i = 0; i < dlen; i += 20 )
    {
        sha1_hmac_mbed( S2, hs, tmp, 20 + nb, h_i );
        sha1_hmac_mbed( S2, hs, tmp, 20,      tmp );
...
void ssl_calc_verify_ssl( ssl_context *ssl, unsigned char hash[36] )
{
    ...
    sha1_update_mbed( &sha1, ssl->session_negotiate->master, 48 );
    sha1_update_mbed( &sha1, pad_1, 40 );
    sha1_finish_mbed( &sha1, hash + 16 );

    sha1_starts_mbed( &sha1 );
    sha1_update_mbed( &sha1, ssl->session_negotiate->master, 48 );
    sha1_update_mbed( &sha1, pad_2, 40 );
    sha1_update_mbed( &sha1, hash + 16, 20 );
    sha1_finish_mbed( &sha1, hash + 16 );
...
void ssl_calc_verify_tls_sha256( ssl_context *ssl, unsigned char hash[32] )
{
    ...
    sha256_finish_mbed( &sha256, hash );
                                       ...
                                       const unsigned char *buf, size_t len )
{
    ...
    sha1_update_mbed( &ssl->handshake->fin_sha1, buf, len );
#endif
#if defined(POLARSSL_SSL_PROTO_TLS1_2)
#if defined(POLARSSL_SHA256_C)
    sha256_update_mbed( &ssl->handshake->fin_sha256, buf, len );
#endif
#if defined(POLARSSL_SHA512_C)
                ...
                ssl_context *ssl, unsigned char *buf, int from )
{
    ...
    sha1_update_mbed( &sha1, (const unsigned char *) sender, 4 );
    sha1_update_mbed( &sha1, session->master, 48 );
    sha1_update_mbed( &sha1, padbuf, 40 );
    sha1_finish_mbed( &sha1, sha1sum );
    ...
    sha1_update_mbed( &sha1, session->master, 48 );
    sha1_update_mbed( &sha1, padbuf , 40 );
    sha1_update_mbed( &sha1, sha1sum, 20 );
    sha1_finish_mbed( &sha1, buf + 16 );
                ...
                ssl_context *ssl, unsigned char *buf, int from )
{
    ...
    sha1_finish_mbed( &sha1, padbuf + 16 );
                ...
                ssl_context *ssl, unsigned char *buf, int from )
{
    ...
    sha256_finish_mbed( &sha256, padbuf );
...
static void ssl_handshake_params_init( ssl_handshake_params *handshake )
{
    ...
    sha1_starts_mbed( &handshake->fin_sha1 );
#endif
#if defined(POLARSSL_SSL_PROTO_TLS1_2)
#if defined(POLARSSL_SHA256_C)
    sha256_init(   &handshake->fin_sha256    );
    sha256_starts_mbed( &handshake->fin_sha256, 0 );
#endif
#if defined(POLARSSL_SHA512_C)
