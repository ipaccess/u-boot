/**
 * @brief Atheros AR8237, AR8238 driver
 * Base source code from Atheros
 * Source code modified by Intel
 *
 *
 */
...
#define S17_LOOKUP_CTRL_REG(_macport)   (S17_P0LOOKUP_CTRL_REG + (_macport * 12))
#define S17_PRI_CTRL_REG(_macport)      (S17_P0PRI_CTRL_REG    + (_macport * 12))
#define S17_LEARN_LMT_REG(_macport)     (S17_P0LEARN_LMT_REG   + (_macport * 12))
...
/* MIB counters */
#define S17_MIB_PORT(_macport)          (0x1000 | (_macport << 8))
...
/** @ brief RGMII settings for T2K RGMII to Atheros Switch */

                                      /*| S17_TX_FLOW_EN     */
                                      /*| S17_RX_FLOW_EN     */
                                      /*| S17_TX_HALF_FLOW_EN */

#define S17_RGMII_PORT_STATUS_DEFAULT   ( S17_SPEED_1000M        \
                                        | S17_TXMAC_EN           \
                                        | S17_RXMAC_EN           \
                                        | S17_DUPLEX_FULL        \
                                        )

#ifdef ATHEROS_SWITCH_LAN_FLOW_CONTROL_ENABLED
#define S17_LAN_PORT_STATUS_DEFAULT    (S17_SPEED_1000M     | \
                                        S17_TXMAC_EN        | \
                                        S17_RXMAC_EN        | \
                                        S17_TX_FLOW_EN      | \
                                        S17_RX_FLOW_EN      | \
...
#ifdef C2K
/** @brief Valid MAC Port for C2K EVM configuration
 *
 * MAC0 0x01 CPU port present
 * MAC1 0x02 PHY port 0 LAN port present
 * MAC2 0x04 PHY port 1 LAN port present
 * MAC3 0x08 PHY port 2 LAN port present
 * MAC4 0x10 PHY port 3 LAN port present
 * MAC5 0x20 PHY port 4 WAN port present (C2K comes from RGMII interface not from switch)
 * MAC6 0x40 PHY port 5 no connection
 *
 * Bit mask: 0x3F
 *
 */
#define VALID_MAC_PORT_BITMASK          0x3fUL
#else
/** @brief Valid MAC Port for T2K EVM configuration
 *
 * MAC0 0x01 CPU port not present (note this may change in future version of board)
 * MAC1 0x02 PHY port 0 LAN port present
 * MAC2 0x04 PHY port 1 LAN port present
 * MAC3 0x08 PHY port 2 LAN port present
 * MAC4 0x10 PHY port 3 LAN port present
 * MAC5 0x20 PHY port 4 WAN port not preset (C2K comes from RGMII interface not from switch)
 * MAC6 0x40 RGMII connection    present
 *
 * Bit mask: 0x5E
 *
 */
#define VALID_MAC_PORT_BITMASK          0x5eUL
#endif
...
#define S17_IGMP_JOIN_LEAVE_DPALL       (VALID_MAC_PORT_BITMASK << S17_IGMP_JOIN_LEAVE_DP_SHIFT)
#define S17_BROAD_DPALL                 (VALID_MAC_PORT_BITMASK << S17_BROAD_DP_SHIFT)
#define S17_MULTI_FLOOD_DPALL           (VALID_MAC_PORT_BITMASK << S17_MULTI_FLOOD_DP_SHIFT)
#define S17_UNI_FLOOD_DPALL             (VALID_MAC_PORT_BITMASK << S17_UNI_FLOOD_DP_SHIFT)
...
/* Lookup control register bits*/
#define S17_LOOKUP_MULTICAST_DROP_EN         (1UL << 31) /* Drop multicast packets except certain IGMP and special DIP packets */
#define S17_LOOKUP_UNI_LEAKY_EN              (1UL << 28) /* Unicast can cross VLANs */
#define S17_LOOKUP_MULTI_LEAKY_EN            (1UL << 27) /* Multicast can cross VLANs */
#define S17_LOOKUP_ARP_LEAKY_EN              (1UL << 26) /* ARP can cross VLANs */
#define S17_LOOKUP_ING_MIRROR_EN             (1UL << 28) /* Ingress port mirroring enabled (frames copied to mirror port) */
#define S17_LOOKUP_PORT_LOOPBACK_EN          (1UL << 21) /* Loopback all frames in back to source */
#define S17_LOOKUP_UNI_LEARN_EN              (1UL << 20) /* Enable learn operation on this port */
#define S17_LOOKUP_PORT_STATE(_state)        (_state << 16)
#define S17_LOOKUP_PORT_STATE_DISABLE        (0UL << 16) /* Disabled port state */
#define S17_LOOKUP_PORT_STATE_BLOCKED        (1UL << 16) /* Non switch management frames blocked */
#define S17_LOOKUP_PORT_STATE_LISTENING      (2UL << 16) /* Listening port state, management frames in/out, normal frames discarded */
#define S17_LOOKUP_PORT_STATE_LEARNING       (3UL << 16) /* Learning port state, allows frames in, but only for learning purposes */
#define S17_LOOKUP_PORT_STATE_FORWARDING     (4UL << 16) /* Normal switch forwarding/learning/etc. state */
#define S17_LOOKUP_FORCE_PORT_VLAN_EN        (1UL << 10) /* Force to use port based VLAN enable */
#define S17_LOOKUP_VLAN_MODE(_mode)          (_mode << 8)
#define S17_LOOKUP_VLAN_MODE_802_1Q_DISABLED (0UL <<  8)
#define S17_LOOKUP_VLAN_MODE_FALLBACK        (1UL <<  8)
#define S17_LOOKUP_VLAN_MODE_CHECK           (2UL <<  8)
#define S17_LOOKUP_VLAN_MODE_SECURE          (3UL <<  8)
#define S17_LOOKUP_PORT_VID_MEM(_portmask)   (_portmask)

/**
 * Default value for normal, for initial testing value,
 * set everything for dumb switch operation, all on the same port based VLAN,
 * set as forwarding state
 * Set port mask to allow other ports execept this one (so frames don't echo back)
 */
#define S17_LOOKUP_SET_PORT_DEFAULT(_port) ( S17_LOOKUP_UNI_LEAKY_EN                    \
                                           | S17_LOOKUP_MULTI_LEAKY_EN                  \
                                           | S17_LOOKUP_ARP_LEAKY_EN                    \
                                           | S17_LOOKUP_UNI_LEARN_EN                    \
                                           | S17_LOOKUP_PORT_STATE_FORWARDING           \
                                           | S17_LOOKUP_FORCE_PORT_VLAN_EN              \
                                           | S17_LOOKUP_VLAN_MODE_802_1Q_DISABLED       \
                                           | ((~(1 << _port)) & VALID_MAC_PORT_BITMASK) \
                                           )
/* Priority control register bits */
#define S17_PRI_CTRL_EG_MAC_BASE_VLAN_EN     (1UL << 20)
#define S17_PRI_CTRL_DA_PRI_EN               (1UL << 19)
#define S17_PRI_CTRL_VLAN_PRI_EN             (1UL << 18)
#define S17_PRI_CTRL_IP_PRI_EN               (1UL << 17)


/** @brief Priority control register default for T2K board, seutp for port based VLAN on on valid ports */
#define S17_PRI_CTRL_DEFAULT               (S17_PRI_CTRL_EG_MAC_BASE_VLAN_EN)
//
// PHY register values
//
...
int          athrs17_phy_setup      (struct mii_device * mdev,
                                     int                 phyUnit
                                    );

int          athrs17_phy_stat       (struct mii_device *mdev
                                    );

unsigned int athrs17_reg_read       (struct mii_device * mdev,
                                     unsigned int        reg_addr
                                    );

void         athrs17_reg_write      (struct mii_device * mdev,
                                     unsigned int        reg_addr,
                                     unsigned int        reg_val
                                    );

unsigned int athrs17_phy_debug_read (struct mii_device * mdev,
                                     unsigned int        phy_addr,
                                     unsigned int        reg_addr
                                    );

void         athrs17_phy_debug_write(struct mii_device * mdev,
                                     unsigned int        phy_addr,
                                     unsigned int        reg_addr,
                                     unsigned int        reg_val
                                    );
